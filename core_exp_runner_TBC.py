...
class CoreRunner:
    def __init__(self, conf, device=torch.device('cuda')):
        ...
        # 루프 완전히 제거
        rd = self.dataset.ref_distances  # [H, W]
        rn = self.dataset.ref_normals    # [H, W, 3]

        write_image(pjoin(self.exp_dir, 'distance_vis_merged.png'),
                    colorize_single_channel_image((rd.min() + 1e-6) / (rd + 1e-6)))
        if rn is not None:
            write_image(pjoin(self.exp_dir, 'normal_vis_merged.png'),
                        (rn * .5 + .5) * 255.)
        ...
        for i in range(self.dataset.n_images):
            self.sup_pool.register_sup_info(pose=self.dataset.poses[i],
                                                ...
                                                normal=self.dataset.ref_normals,
                                                log=True)
    ...
    def train(self, raw_only=False):
        ...
        for anchor_idx in range(n_anchors):
            ...
            inpaint_mask = torch.maximum(inpaint_mask, (distances.squeeze() < 0.1).float())
    ...
    def inpaint_new_panorama(self, phase, anchor_idx, colors, distances, mask):
        ...
        if self.conf.rgbd_inpaint:
            ...
            inpainted_img1 = self.inpainter.inpaint(colors, mask, exp_dir=self.exp_dir, 
                                        anchor_idx=anchor_idx, phase=phase)
            inpainted_img1 = inpainted_img1.cuda()
            inpainted_img2 = self.inpainter.inpaint(colors, mask, exp_dir=self.exp_dir,
                                         anchor_idx=anchor_idx, phase=phase)
            inpainted_img2 = inpainted_img2.cuda()
            write_image(pjoin(vis_dir, 'inpainted1_{}.jpg'.format(phase)), inpainted_img1 * 255.)
            write_image(pjoin(vis_dir, 'inpainted2_{}.jpg'.format(phase)), inpainted_img2 * 255.)
            ...
            inpainted_img1,                    # img1
            inpainted_img2,                    # img2 ← 동일 이미지로 채움
            ref_distance1=distances,          # 기존 distance를 초기값으로
            ref_distance2=distances,          # 동일하게
            mask=mask.squeeze()[..., None],   # [H, W, 1]로 shape 맞추기
            reg_loss_weight=0.,
            normal_loss_weight=5e-2,
            normal_tv_loss_weight=5e-2
        )
        ...
        height, width, _ = inpainted_img1.shape
        ...
        write_image(pjoin(vis_dir, 'aligned_disparity_{}.jpg'.format(phase)),
            colorize_single_channel_image(
                inpainted_distances.min().item() / (inpainted_distances + 1e-6)))

        return inpainted_img1, inpainted_distances, inpainted_normals
    ...
    def extract_panorama(self, out_dir="anchor_images"):
        ...
        os.makedirs(pjoin(self.exp_dir, out_dir, 'middle'), exist_ok=True)
        os.makedirs(pjoin(self.exp_dir, out_dir, 'up'), exist_ok=True)
        os.makedirs(pjoin(self.exp_dir, out_dir, 'down'), exist_ok=True)

        device = self.device
        target = self.pose_sampler.anchor_pts
        ...
        for i, t in enumerate(target):
            ...
            if abs(z) < 1e-6:
                path = pjoin(self.exp_dir, out_dir, 'middle', filename)
            elif z > 0:
                path = pjoin(self.exp_dir, out_dir, 'up', filename)
            else:
                path = pjoin(self.exp_dir, out_dir, 'down', filename)
