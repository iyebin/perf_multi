...
class Dataset:
    ...
    def save_ref_geometry(self):
        ...
        # Save point cloud
        pano_dirs = img_coord_to_pano_direction(img_coord_from_hw(self.height, self.width)) #just directions
        pts = pano_dirs * self.ref_distance.squeeze()[..., None] #utilize ref_distance
    ...
    def ref_point_cloud(self):
        ...
        '''
        pano_dirs = img_coord_to_pano_direction(img_coord_from_hw(self.height, self.width)) # 픽셀 위치를 방향벡터로 변환
        pts = pano_dirs * self.ref_distance.squeeze()[..., None] # 점은 방향벡터와 거리의 곱
        return pts
