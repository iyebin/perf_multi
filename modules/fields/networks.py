...
class VanillaMLP(nn.Module):
    ...
    def make_linear(self, dim_in, dim_out, is_first, is_last):
        ...
        if self.sphere_init:
            if is_last:
                torch.nn.init.constant_(layer.bias, -self.sphere_init_radius) #layer의 bias를 -0.5로 채움 => 반지름 0.5 구현
                torch.nn.init.normal_(layer.weight, mean=np.sqrt(np.pi) / np.sqrt(dim_in), std=0.0001) #layer의 weight를 정규분포로 초기화함, std > 0.0001 : 원점에서 멀어질수록 값이 커짐 (거리함수와 유사)
            elif is_first:
                torch.nn.init.constant_(layer.bias, 0.0) #layer의 bias를 0으로 채움
    ...
    def make_activation(self):
        if self.sphere_init:
            return nn.Softplus(beta=100) #activation : softplus
        else:
