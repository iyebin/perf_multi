...
def _resize_pos_embed(self, posemb, gs_h, gs_w):
    ...
    # print(f"posemb_tok : {posemb_tok}")
    # print(f"posemb_grid : {posemb_grid}")
...
def _make_vit_b_rn50_backbone(
    ...
    size=[384, 384],
    hooks=[0, 1, 8, 11], #transformer block 중 feature를 뽑을 위치
    vit_features=768,
    use_vit_only=False,
    use_readout="ignore",
    start_index=1,
):
    pretrained = nn.Module() #기존 ViT 모델을 감싸는 wrapper를 만듦.

    pretrained.model = model
    print("-----------vit_base_resnet50_384---------")
    print("-----------pretrained model-------------")
    print(model)

    if use_vit_only == True:
         #forward pass를 하는 동안 (output이 계산할 때 마다) 만들어놓은 hook function을 호출.
        pretrained.model.blocks[hooks[0]].register_forward_hook(get_activation("1")) #activation dictionary에 0번째 block의 output 저장
        pretrained.model.blocks[hooks[1]].register_forward_hook(get_activation("2")) #activation dictionary에 1번째 block의 output 저장
    else:
...
def _make_pretrained_vitb_rn50_384(
    pretrained, #True
    use_readout="ignore", hooks=None, use_vit_only=False
):
    ...
    return _make_vit_b_rn50_backbone(
        ...
        use_readout=use_readout, #project
    )
