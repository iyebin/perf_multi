...
class PanoFusionInvPredictor(GeoPredictor):
    ...
    def inpaint_distance(self, img, ref_distance, mask, gen_res=384):
        ...
        # pers_imgs: [n_pers, 3, H, W]
        for i in range(pers_imgs.shape[0]):
            img_i = pers_imgs[i].detach().cpu()  # [3, H, W]

            # 값 범위가 [0,1]이 아니면 clamp
            img_i = img_i.clamp(0, 1)

            save_path = os.path.join(save_dir, f"pers_{i:02d}.png")
            save_image(img_i, save_path)

        print(f"Saved {pers_imgs.shape[0]} perspective images to {save_dir}")
