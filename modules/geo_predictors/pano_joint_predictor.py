...
class PanoJointPredictor(GeoPredictor):
    ...
    def __call__(self, img, ref_distance, mask, gen_res=384,
        ...
        # save_dir = "./debug_pers_imgs"
        # os.makedirs(save_dir, exist_ok=True)

        # for i in range(pers_imgs.shape[0]):
        #     img_i = pers_imgs[i].detach().cpu()  # [3, H, W]
           

        #     # 값 범위 보정 (중요)
        #     img_i = img_i.clamp(0, 1)
            
        #     save_image(img_i, os.path.join(save_dir, f"pers_{i:03d}.png"))

        # print(f"[DEBUG] saved {pers_imgs.shape[0]} perspective images to {save_dir}")
        ...
        scale_params = torch.zeros([n_pers], requires_grad=True) #depth scale
        ...
        for phase in ['global', 'hybrid']:
            for iter_step in tqdm(range(all_iter_steps)):
                ...
                #cur_sup_info에서 dirs 추출 후, 채널 순서 변경
                dirs = cur_sup_info[:, :3, :, 0].permute(0, 2, 1)                                    # [n_pers, local_batch_size, 3]
                #unit vector로 만듦. 
                dirs = dirs / torch.linalg.norm(dirs, 2, -1, True)

                #cur_sup_info에서 distance 추출 후, 채널 순서 변경
                #모델에서 예측한 distance 값 (perspective)
                #ex) ref_pred_distances[3, 10] : 3번 perspective의 10번째 sampled ray depth
