...
def _resample_uniformly(pts):
    """ pts를 균일하게 resample하는 함수.

        길이가 n인 pts를 128n으로 늘렸다가, 다시 균일하게 n개의 pts로 반환 (bias_len에 누적길이 저장)
...
class CirclePoseSampler(PoseSampler):
    ...
    """pose를 생성하는 sampler 클래스

    PoseSampler를 상속받은 subclass. 
    traverse_ratios와 n_anchors_per_ratio를 받아서 새로운 시점의 카메라 포인트를 샘플링한다.
    tranjactory 문제를 해결하기 위해 distance map을 반영해 최대 경로를 지정해서 사용한다.

    Args:
        distance_map(torch.Tensor): 물체와 카메라 사이의 거리 2d map
        traverse_ratios(list): 폐곡선의 반지름 결정. [0.2, 0.4, 0.6]
        n_anchors_per_ratio(list): 각 폐곡선마다 sampling할 점의 수. [8, 8, 8]
        test_z_min_max(tuple): 
    
    Attributes:
        plane_pts_raw    (tensor):
        plane_pts_filter (tensor):
        plane_pts_smooth (tensor):
        anchor_pts       (tensor): 
        traverse_pts     (tensor):
        traverse_normals ():
        n_anchors        ():
    ...
    """
    def __init__(self, distance_map, traverse_ratios, n_anchors_per_ratio, pitch_deg, pitch_band_deg,
                 test_z_min_max=(0., 0.), **kwargs):
        super().__init__()
        if torch.is_tensor(distance_map):
            distance_map = distance_map.cpu().numpy()
        distance_map = distance_map.squeeze() # (H, W)

        height, width = distance_map.shape
        pano_coords = img_to_pano_coord(img_coord_from_hw(height, width)) #(H, W, 2)
                                                                          #pitch: [..., 0]
                                                                          #yaw: [..., 1]

        #pitch setting
        self.pitch = np.deg2rad(pitch_deg)
        self.pitch_band = np.deg2rad(pitch_band_deg)
        
        pitch_map = pano_coords[:, :, 0].cpu().numpy()
        mask = np.abs(pitch_map - self.pitch) < self.pitch_band

        #plane_dis slicing
        plane_dis = distance_map * np.cos(pano_coords[:, :, 0].cpu().numpy()) # 실제 거리로 변환
        plane_dis = np.where(mask, plane_dis, np.nan)
        plane_dis[np.where(plane_dis < 1e-5)] = 1e9
        plane_dis = np.nanmin(plane_dis, axis=0)

        # h_height = height // 2 # 중간 높이
        # plane_dis = plane_dis[h_height - 10: h_height + 10] #pitch=0인 부분만 살려둠. 중앙부분만 슬라이싱
        # plane_dis[np.where(plane_dis < 1e-5)] = 1e9 #너무 작은 값은 무한대로 처리
        # plane_dis = np.min(plane_dis, axis=0) #가장 작은 plane_dis의 값만 살림

        for i in range(1, width):
            if plane_dis[i] > 1e8:  #무한대면, 
                plane_dis[i] = plane_dis[i - 1] #직전 값과 같도록

        for i in range(1, width):
            if plane_dis[width - i - 1] > 1e8: #뒤에서부터 무한대면
                plane_dis[width - i - 1] = plane_dis[width - i] #직후 값과 같도록
        ...
        plane_coords = torch.stack([torch.ones([width]) * .5,
                                    torch.linspace(.5 / width, 1. - .5 / width, width)], -1) #카메라가 이동할 수 있는 최대 거리
