...
class NeRFOCCRenderer(nn.Module):
    ...
    def render(self,
        ...
        #ray indices, t_starts, t_ends 확인
        # breakpoint()
        ...
        # expected_pts 계산(추가)
        # Expected surface position: weighted average of 3D sample positions.
        # Used for PanoGRF-style cross-view consistency (see SupInfoPool.query_ibr_colors).
        sample_positions = (
            rays_o[ray_indices] + rays_d[ray_indices] * sampled_distances
        )  # [M, 3]
        expected_pts = accumulate_along_rays(
            weights, sample_positions, ray_indices=ray_indices, n_rays=n_rays
        )  # [N, 3]
