...
def colorize_single_channel_image(image, color_map=cv.COLORMAP_JET):
    ...
    return numpy data
    '''
    image = image.squeeze() #1의 값을 가지는 axis 제거
    assert len(image.shape) == 2

    image = (image - image.min()) / (image.max() - image.min() + 1e-6) * 255 #정규화
    if torch.is_tensor(image):
        image = image.cpu().numpy()

    image = image.astype(np.uint8)

    image = cv.applyColorMap(image, color_map) #단일 채널 이미지를 컬러 이미지로 바꾸는 opencv 함수
